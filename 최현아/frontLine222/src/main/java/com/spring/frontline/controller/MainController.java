...
//			model.addAttribute("userEmail2", userEmail);
	...
	public String checkNumber(@RequestParam int putKey, @RequestParam String userEmail2, HttpServletRequest request, Model model, UserDTO dto) {
		HttpSession session = request.getSession();
		// 세션에 원래 이메일 정보 저장 하기.
...
//			model.addAttribute("finalCheck", "7");
	...
	public String pwWay(Model model, HttpServletRequest request) {
		// 모델에 담아서 보내기 셀렉트된 이메일 보내기
//		HttpSession session = request.getSession();
...
//		model.addAttribute("userEmail", userEmail);
	...
	public String setNewPw(Model model, HttpServletRequest request) {
//		HttpSession session = request.getSession();
//		String userEmail = (String) session.getAttribute("userEmail");
//		System.out.println("/setNewPw 에서 userEmail 값: " + userEmail);
//		model.addAttribute("emailDto", userEmail);
		return "/find/set_new_password";
	}
	
	@RequestMapping("inputNewPw")
	public String inputNewPw(@RequestParam String setUserPw, UserDTO dto, Model model, HttpServletRequest request) {
...
//		System.out.println("userEmail2 : " + userEmail2);
		...
		if(setUserPw.equals(dto.getUserPw())) {
			...
			return "/find/complete_set_new_password";
		}
//		HttpSession session = request.getSession();
//		String userEmail = (String) session.getAttribute("userEmail");
//		System.out.println("/setNewPw 에서 userEmail 값: " + userEmail);
